        ...
        <div className="col-md-8" style={{ textAlign: 'center' }}>
          ...
          <div className="row">
              ...
              {/** `cinemaList` 데이터가 배열 형태로 전달된다고 가정 */}
              {
                ...
                cinemaList && cinemaList.map((cinema) => (
                  <div className="card" style={{ border: 'none', fontSize: '20px' }} key={cinema.id}>
                    <Link to={`/admin/theater/list/${cinema.id}`}>
                      <img
                    ...
                    <Link to={`/admin/cinema/enter?id=${cinema.id}`}>
            ...
            <div className="container mt-4">
              <form action="/admin" className="d-flex" method="get">
                <input
                ...
                {/* 페이지네이션 - Link */}
                {
                  ...
                  (
                    <div className="pagination">
                      <Link to={`/admin?page=${pagination.first}&search=${search || ''}&size=${pagination.size}`} className={ResetCs.btnPPage}>
                        <KeyboardDoubleArrowLeftIcon />
                      </Link>
                      {
                        ( pagination.page <= pagination.first )
                        ||
                        <Link to={`/admin?page=${pagination.prev}&search=${search || ''}&size=${pagination.size}`} className={ResetCs.btnPPage}>
                      ...
                      }
                      {
                        pageList.map( page => (
                          <div key={page}>
                            <Link className={`${ResetCs.btnPPage} ${page === pagination.page ? ResetCs.activI : ''}`}  to={`/admin?page=${page}&search=${search || ''}&size=${pagination.size}`} >{page}</Link>
                          </div>
                        ))
                      }
                      {
                        ...
                        ||
                        <Link to={`/admin?page=${pagination.next}&search=${search || ''}&size=${pagination.size}`} className={ResetCs.btnPPage}>
                          <KeyboardArrowRightIcon />
                        </Link>
                      }
                      <Link to={`/admin?page=${pagination.last}&search=${search || ''}&size=${pagination.size}`} className={ResetCs.btnPPage}>
