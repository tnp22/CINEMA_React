...
const InquiryListForm = ({ inquiryList, option, keyword, handleSearch }) => {
  ...
  const checkType = (inquiry) => {
    if (isAdmin || isInquiry) {
      navigate(`/inquiry/select/${inquiry.id}`);
      return;
    }

    if (inquiry.type === 0) {
      setSelectedInquiryId(inquiry.id);
      setActualPassword(inquiry.password);
      setIsModalOpen(true);
    } else {
      navigate(`/inquiry/select/${inquiry.id}`);
    }
  ...
  const confirmPassword = () => {
    if (inputPassword === actualPassword) {
      closeModal();
      navigate(`/inquiry/select/${selectedInquiryId}/${actualPassword}`);
    } else {
      setShowError(true);
    }
  };
  ...
  return (
    <div style={{ minHeight: '450px', marginBottom:'50px' }}>
      ...
      <div className="container">
        ...
        <table className="custom-table mb-5">
            ...
            {inquiryList && inquiryList.list && inquiryList.list.length > 0 && inquiryList.list.map((inquiry) => (
                ...
                <td className="text-start">
                    ...
                    >
                    {inquiry.type === 0 && <FontAwesomeIcon icon={faLock} className="me-2" />}
                    {inquiry.title}
                    </button>
                </td>
            ...
            <Link to="/inquiry/insert" className="btn custom-btn float-end">작성</Link>
        {/* Pagination */}
        {inquiryList.pages > 0 && (
        ...
        <Modal show={isModalOpen} onHide={closeModal} centered className='inquiry-modal'> 
            ...
            <Modal.Body>
                ...
                />
                {showError && <small className="text-danger">비밀번호가 틀렸습니다.</small>}
            </Modal.Body>
            <Modal.Footer>
                <Button variant="secondary" onClick={confirmPassword}>확인</Button>
            </Modal.Footer>
        </Modal>
      </div>
    </div>
